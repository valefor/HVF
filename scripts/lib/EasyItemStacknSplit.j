...
//  by Dangerb0y
//=====================================================================================
library EasyItemStacknSplit initializer init
//=====================================================================================
//
...
globals
    ...
    // Trigger
    private trigger trigEasyItemStacknSplit
endglobals
//=====================================================================================
...
//=====================================================================================
    ...
    globals
        ...
        private trigger trigCallBack
    endglobals
    
    //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // PUBLIC FUNCTION : isUnitInventoryFull( unit )
    ...
    function UnitStackItem takes unit u, item i returns boolean
        ...
        //call BJDebugMsg("[1]Item:"+GetItemName(i)+", Charges:"+I2S(GetItemCharges(i)))
        if ic <= 0 then
            // If not we just give it to the unit
            call DisableTrigger( trigEasyItemStacknSplit )
            //call BJDebugMsg("[2]Item:"+GetItemName(i)+", Charges:"+I2S(GetItemCharges(i)))
            call UnitAddItem( u, i )
            call EnableTrigger( trigEasyItemStacknSplit )
        else
            // Set vars
    ...
    private function ActionController takes nothing returns boolean
        ...
        if not TriggerEvaluate(trigCallBack) then
            return false
        endif
                    ...
                    if UnitHasItem(u, i) then
                        ...
                        if GetItemTypeId(ii) == GetItemTypeId(i) then
                            if ii == i then
                                // Split items
                                if SPLIT then
                                    set full = isUnitInventoryFull( u )
    ...
    //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // CONTROLLER : RegisterCb
    //  Register callback, evaluate callback before performing ActionController().
    //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    public function RegisterCb takes boolexpr cb returns triggercondition
        return TriggerAddCondition(trigCallBack, cb)
    endfunction
    ...
    private function init takes nothing returns nothing
        ...
        local integer x = 0
        set trigEasyItemStacknSplit = CreateTrigger()
        set trigCallBack = CreateTrigger()
        loop
            call TriggerRegisterPlayerUnitEvent( trigEasyItemStacknSplit, Player(x), EVENT_PLAYER_UNIT_PICKUP_ITEM, null )
            call TriggerRegisterPlayerUnitEvent( trigEasyItemStacknSplit, Player(x), EVENT_PLAYER_UNIT_ISSUED_TARGET_ORDER, null )
        ...
        call TriggerAddCondition( trigEasyItemStacknSplit, function ActionController )
